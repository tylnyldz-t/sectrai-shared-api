...
- Owner actor headers are also canonical and never trimmed into a different maker/checker identity; even a programmatic `liveOptInRequested: false` surface disables the connector, and a successful artifact run must have a review expiry strictly after its canonical run instant.
