...
/** A durable, per-product/workspace SHA-256 audit chain using the existing Record backbone. */
  ...
  async append(event: ConnectorAuditEvent): Promise<{ hash: string }> {
    return this.prisma.$transaction(async (transaction) => {
      ...
      await transaction.record.create({
        data: { product: event.product, workspaceId: event.workspaceId, moduleId: GCL_AUDIT_MODULE_ID, values: { event, previousHash, hash }, status: 'append-only', createdBy: 'gcl-audit' },
      })
      return { hash }
    })
  }
...
/** Test-only audit seam. Application wiring must use the durable implementation. */
