...
import { appendVerifiedJncAuditEvent } from './jnc-audit.js'
...
export type JncRunConnectorRequest = {
  connectorId: string
  input: unknown
  product: string
  workspaceId: string
...
function registerConnector(value: Connector): RegisteredConnector {
  try {
    ...
    const exactBuiltinBinding = (id === 'text-to-3d' || id === 'image-text-to-3d')
      ? kind === 'media-3d' && scopes?.length === 1 && scopes[0] === '3d:generate'
      : id === 'game-engine' && kind === 'game-engine' && scopes?.length === 1 && scopes[0] === 'game:project:build'
    if (typeof id !== 'string' || !/^[a-z0-9][a-z0-9-]{0,79}$/.test(id) ||
      (kind !== 'media-3d' && kind !== 'game-engine') || authKind !== 'owner-approval' ||
      !scopes || !exactBuiltinBinding || scopes.some((scope) => !scope || scope.length > 120) || new Set(scopes).size !== scopes.length ||
...
export class JncConnectorRegistry {
 ...
 */
export class JncGovernedConnectorRunner {
  constructor(
    private readonly registry: JncConnectorRegistry,
    private readonly auditLog: AuditLog,
    private readonly quota: ConnectorQuota,
    private readonly now: () => Date = () => new Date(),
  ) {}

  async run(request: JncRunConnectorRequest): Promise<ConnectorResult> {
    ...
    const context = Object.freeze({
      ...
      now: runTime.now,
    }) as ConnectorRunContext & { actor: string }
    await connector.preflight?.(input, context)
    const requestedAudit = await appendVerifiedJncAuditEvent(this.auditLog, {
    ...
    try {
      await this.quota.consume({
        ...
        quotaGroup: connector.kind === 'media-3d' ? 'three-d' : 'game-engine',
        requestedItems: context.requestedItems,
        occurredAt: runTime.occurredAt,
      })
      const result = validatedSyntheticConnectorResult(await connector.run(input, context), connector.id)
      const succeededAudit = await appendVerifiedJncAuditEvent(this.auditLog, {
      ...
      await appendVerifiedJncAuditEvent(this.auditLog, {
