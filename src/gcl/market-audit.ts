...
export function marketHashAuditEvent(event: ConnectorAuditEvent, previousHash: string | null): string {
...
/**
 * A durable, per product/workspace SHA-256 audit chain stored in the existing
 * Record backbone. GCL reserves this module so normal record routes cannot
 * alter the chain. A deployment must retain the same database for continuity.
 */
export class MarketPrismaHashChainAuditLog implements AuditLog {
  constructor(private readonly prisma: PrismaClient) {}

  async append(event: ConnectorAuditEvent): Promise<{ hash: string }> {
    return this.prisma.$transaction(async (transaction) => {
      // Serialize each tenant chain without a schema migration; otherwise two
      // concurrent app instances could append from the same predecessor.
      ...
      const hash = marketHashAuditEvent(event, previousHash)
...
/** Test-only seam: it is deliberately not used by the application factory. */
export class MarketInMemoryHashChainAuditLog implements AuditLog {
