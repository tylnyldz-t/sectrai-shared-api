import { appendVerifiedAuditEvent } from './camera-audit.js'
...
export type CameraRunConnectorRequest = {
  connectorId: string
  input: unknown
  product: string
  workspaceId: string
 ...
 */
function governedRunRequest(value: unknown): CameraRunConnectorRequest {
 ...
 */
function governedRunContext(request: CameraRunConnectorRequest, occurredAt: Date): ConnectorRunContext {
  const uniqueScopes = new intrinsicSet<string>()
  const scopes: string[] = []
...
function governedRunnerCollaborators(registry: CameraConnectorRegistry, auditLog: AuditLog, quota: ConnectorQuota, now: () => Date): GovernedRunnerCollaborators {
...
export class CameraGovernedConnectorRunner {
  readonly #collaborators: GovernedRunnerCollaborators

  constructor(registry: CameraConnectorRegistry, auditLog: AuditLog, quota: ConnectorQuota, now: () => Date = () => new Date()) {
  ...
  async run(request: CameraRunConnectorRequest): Promise<ConnectorResult> {
