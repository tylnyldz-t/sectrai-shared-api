...
export interface Connector<TInput = unknown, TData = unknown> {
  ...
  scopes: readonly string[]
  /** Non-mutating configuration/policy gate, executed before audit and quota reservation. */
...
export type ConnectorAuditEvent = {
  type: 'connector.run.requested' | 'connector.run.succeeded' | 'connector.run.failed' | 'connector.artifact.owner_liked'
  connectorId: string
  product: string
  workspaceId: string
  actor: string
