...
/** A failed run remains reserved: a later real adapter may have incurred cost already. */
...
/** Missing limits deliberately disable both synthetic voice connector routes. */
export function voiceDailyQuotaFromEnvironment(environment: NodeJS.ProcessEnv = process.env): DailyQuotaConfig {
  const dailyRuns = positiveInteger(environment.GCL_VOICE_DAILY_RUN_QUOTA)
  const dailyItems = positiveInteger(environment.GCL_VOICE_DAILY_ITEM_QUOTA)
  if (!dailyRuns || !dailyItems) throw new ConnectorUnavailableError('VOICE_CONNECTOR_QUOTA_NOT_CONFIGURED')
...
export class EnvironmentPrismaDailyConnectorQuota implements ConnectorQuota {
  ...
  async consume(request: Parameters<ConnectorQuota['consume']>[0]): Promise<void> {
    return new PrismaDailyConnectorQuota(this.prisma, voiceDailyQuotaFromEnvironment(this.environment)).consume(request)
  }
}
