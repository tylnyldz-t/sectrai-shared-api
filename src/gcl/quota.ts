...
function quotaEnvironmentPrefix(quotaGroup: string): 'GCL_VIDEO' | null {
  if (quotaGroup === 'video') return 'GCL_VIDEO'
  return null
}

export function dailyQuotaFromEnvironment(environment: NodeJS.ProcessEnv = process.env, quotaGroup = 'video'): DailyQuotaConfig {
