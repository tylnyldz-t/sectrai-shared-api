...
export class PrismaDailyConnectorQuota implements ConnectorQuota {
  constructor(private readonly prisma: GclPersistence, private readonly config: DailyQuotaConfig) {}
  ...
  async consume({ product, workspaceId, connectorId, requestedItems, occurredAt }: Parameters<ConnectorQuota['consume']>[0]): Promise<void> {
    ...
    await this.prisma.$transaction(async (transaction) => {
      await transaction.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`${product}:${workspaceId}:${connectorId}:${GCL_VISION_USAGE_MODULE_ID}`}))`
      const records = await transaction.record.findMany({
        where: { product, workspaceId, moduleId: GCL_VISION_USAGE_MODULE_ID, createdAt: { gte: dayStart } },
        select: { values: true },
      })
      ...
      await transaction.record.create({
        data: { product, workspaceId, moduleId: GCL_VISION_USAGE_MODULE_ID, values: { connectorId, requestedItems, occurredAt: occurredAt.toISOString(), state: 'reserved' }, status: 'reserved', createdBy: 'gcl-vision-quota' },
      })
    })
  }
}
