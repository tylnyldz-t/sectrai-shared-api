...
/** A failed run remains reserved; a future real adapter may incur cost before failing. */
...
export class EnvironmentPrismaDailyConnectorQuota implements ConnectorQuota {
  ...
  async consume(request: Parameters<ConnectorQuota['consume']>[0]): Promise<void> {
    return new PrismaDailyConnectorQuota(this.prisma, translationDailyQuotaFromEnvironment(this.environment)).consume(request)
  }
}
