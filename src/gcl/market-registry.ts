...
export type MarketRunConnectorRequest = {
  connectorId: string
  input: unknown
  product: string
  workspaceId: string
...
type SnapshottedMarketRunConnectorRequest = Omit<MarketRunConnectorRequest, 'scopes' | 'ownerApproved'> & { scopes: string[]; ownerApproved: unknown }
...
function snapshotRunRequest(value: unknown): SnapshottedMarketRunConnectorRequest {
 ...
 */
function governedRunnerCollaborators(registry: MarketConnectorRegistry, auditLog: AuditLog, quota: ConnectorQuota, now: () => Date): GovernedRunnerCollaborators {
...
export class MarketConnectorRegistry {
...
export class MarketGovernedConnectorRunner {
  readonly #collaborators: GovernedRunnerCollaborators

  constructor(registry: MarketConnectorRegistry, auditLog: AuditLog, quota: ConnectorQuota, now: () => Date = () => new Date()) {
    this.#collaborators = governedRunnerCollaborators(registry, auditLog, quota, now)
  }

  async run(request: MarketRunConnectorRequest): Promise<ConnectorResult> {
