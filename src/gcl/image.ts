...
import type { Connector, ConnectorResult, ImageConnectorRunContext } from './types.js'
...
export type ImageCandidateScope = Pick<ImageConnectorRunContext, 'product' | 'workspaceId' | 'correlationId'>
export type ImageOwnerReviewContext = Pick<ImageConnectorRunContext, 'product' | 'workspaceId' | 'correlationId' | 'now'>
export type ImageCandidateIssuanceContext = Pick<ImageConnectorRunContext, 'product' | 'workspaceId' | 'actor' | 'correlationId' | 'now'>
...
function requireContext(value: unknown, error: string): ImageConnectorRunContext {
  ...
  return value as unknown as ImageConnectorRunContext
}
...
function snapshotSyntheticImageCandidate(candidate: SyntheticImageCandidate): SyntheticImageCandidate {
  assertSyntheticImageCandidate(candidate)
  const scope = Object.freeze({ ...candidate.scope })
  const safety = Object.freeze({ ...candidate.safety })
  const dispatch = Object.freeze({ ...candidate.creativeWorkerPlan.dispatch })
  const creativeWorkerPlan = Object.freeze({
    ...candidate.creativeWorkerPlan,
    graphShape: Object.freeze([...candidate.creativeWorkerPlan.graphShape]),
    dispatch,
  })
  const ownerReview = Object.freeze({ ...candidate.ownerReview })
  ...
  assertSyntheticImageCandidate(snapshot)
  return snapshot
}
...
export class SyntheticImageTtiConnector implements Connector<TextToImageInput, TextToImageData> {
  ...
  private configured(context: ImageConnectorRunContext): { reviewTtl: number; policy?: FamilySafetyFilter } {
  ...
  preflight(input: TextToImageInput, context: ImageConnectorRunContext): TextToImageInput {
    ...
    familySafetyAssessment(policy, normalized)
    return Object.freeze({
      prompt: normalized.prompt,
      ...(normalized.negativePrompt ? { negativePrompt: normalized.negativePrompt } : {}),
      width: normalized.width,
      height: normalized.height,
    })
  }

  async run(input: TextToImageInput, context: ImageConnectorRunContext): Promise<ConnectorResult<TextToImageData>> {
    const runContext = requireContext(context, 'INVALID_IMAGE_TTI_CONTEXT')
...
export async function ownerLikeSyntheticImage(candidate: SyntheticImageCandidate, ownerApproved: boolean, actor: string, reviewLedger: ImageOwnerReviewLedger, candidateLedger: ImageCandidateLedger, context: ImageOwnerReviewContext): Promise<OwnerLikedImageArtifact> {
  const snapshot = snapshotSyntheticImageCandidate(candidate)
  const artifactId = `owner-liked-${snapshot.candidateId}`
  const decision = await ownerDecision('connector.artifact.owner_liked', snapshot, ownerApproved, actor, reviewLedger, candidateLedger, context, { artifactId, ownerReview: 'liked' })
  return { artifactId, candidateId: snapshot.candidateId, mediaType: snapshot.mediaType, previewDataUri: snapshot.previewDataUri, syntheticUri: snapshot.syntheticUri, ownerReview: { status: 'liked', visibility: 'owner-only', publication: 'blocked', required: true, reviewExpiresAt: snapshot.ownerReview.reviewExpiresAt, actor, occurredAt: decision.occurredAt.toISOString() }, publication: 'blocked', auditHash: decision.auditHash, issuanceAuditHash: decision.issuance.issuanceAuditHash, runAuditHash: decision.issuance.runAuditHash }
}
...
export async function ownerRejectSyntheticImage(candidate: SyntheticImageCandidate, ownerApproved: boolean, actor: string, reason: ImageRejectionReason, reviewLedger: ImageOwnerReviewLedger, candidateLedger: ImageCandidateLedger, context: ImageOwnerReviewContext): Promise<OwnerRejectedImageReview> {
  ...
  const snapshot = snapshotSyntheticImageCandidate(candidate)
  const reviewId = `owner-rejected-${snapshot.candidateId}`
  const decision = await ownerDecision('connector.artifact.owner_rejected', snapshot, ownerApproved, actor, reviewLedger, candidateLedger, context, { reviewId, ownerReview: 'rejected', reason })
  return { reviewId, candidateId: snapshot.candidateId, ownerReview: { status: 'rejected', visibility: 'owner-only', publication: 'blocked', required: true, reviewExpiresAt: snapshot.ownerReview.reviewExpiresAt, actor, occurredAt: decision.occurredAt.toISOString(), reason }, publication: 'blocked', auditHash: decision.auditHash, issuanceAuditHash: decision.issuance.issuanceAuditHash, runAuditHash: decision.issuance.runAuditHash }
}
