...
import { AuditChainError, AuditReceiptError } from './errors.js'
...
export const GCL_AUDIT_MODULE_ID = 'gcl-jnc-audit'
...
type VerifiedAuditAppendReceipt = Readonly<{ hash: string; previousHash: string | null }>
...
/**
 * Treat an append return value as a local witness, not a trusted assertion.
 * The exact receipt must re-hash the event handed to the audit collaborator
 * before quota, connector execution, or result egress can continue.
 */
export async function appendVerifiedJncAuditEvent(
  auditLog: AuditLog,
  event: ConnectorAuditEvent,
): Promise<VerifiedAuditAppendReceipt> {
  const safeEvent = auditEvent(event)
  if (!safeEvent) throw new AuditReceiptError()
  const receipt = ownDataRecord(await auditLog.append(safeEvent))
  if (!receipt || !exactKeys(receipt, ['hash', 'previousHash']) ||
    typeof receipt.hash !== 'string' || !/^[a-f0-9]{64}$/.test(receipt.hash) ||
    (receipt.previousHash !== null && (typeof receipt.previousHash !== 'string' || !/^[a-f0-9]{64}$/.test(receipt.previousHash))) ||
    receipt.hash !== hashAuditEvent(safeEvent, receipt.previousHash)) {
    throw new AuditReceiptError()
  }
  return Object.freeze({ hash: receipt.hash, previousHash: receipt.previousHash })
}
...
export class JncPrismaHashChainAuditLog implements AuditLog {
  constructor(private readonly prisma: PrismaClient) {}

  async append(event: ConnectorAuditEvent): Promise<VerifiedAuditAppendReceipt> {
    const safeEvent = auditEvent(event)
    ...
    return this.prisma.$transaction(async (transaction) => {
      ...
      return Object.freeze({ hash, previousHash })
    })
  }
}

/** Test-only seam; production wiring always uses the durable audit log. */
export class JncInMemoryHashChainAuditLog implements AuditLog {
