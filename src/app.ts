...
import { voiceConnectorsFromEnvironment } from './gcl/voice.js'
import { GCL_VOICE_ARTIFACT_MODULE_ID, PrismaVoiceArtifactStore, type VoiceArtifactRecord } from './gcl/voice-artifacts.js'
import { serializeRecord } from './types.js'
import { connectorRunFrom, mutationFrom, scopeFrom, voiceArtifactApprovalFrom, workspaceScopeFrom } from './validation.js'

type ConnectorRunner = { run(request: RunConnectorRequest): Promise<ConnectorResult> }
type VoiceArtifactStore = {
  propose(input: { product: string; workspaceId: string; actor: string; connectorId: string; proposal: NonNullable<ConnectorResult['artifact']>; runAuditHash: string }): Promise<VoiceArtifactRecord>
  get(product: string, workspaceId: string, id: string): Promise<VoiceArtifactRecord | null>
  decide(input: { product: string; workspaceId: string; id: string; actor: string; decision: 'approved' | 'rejected'; now: Date }): Promise<VoiceArtifactRecord | null>
}
type AppOptions = { prisma?: PrismaClient; now?: () => Date; gclRunner?: ConnectorRunner; gclOwnerToken?: string; voiceArtifactStore?: VoiceArtifactStore }

const GCL_RESERVED_MODULES = new Set([GCL_AUDIT_MODULE_ID, GCL_USAGE_MODULE_ID, GCL_VOICE_ARTIFACT_MODULE_ID])
...
export function createApp({ prisma = new PrismaClient(), now = () => new Date(), gclRunner, gclOwnerToken = process.env.GCL_OWNER_TOKEN, voiceArtifactStore }: AppOptions = {}) {
  const app = express()
  const auditLog = new PrismaHashChainAuditLog(prisma)
  const governedRunner = gclRunner ?? new GovernedConnectorRunner(
    new ConnectorRegistry(voiceConnectorsFromEnvironment()),
    auditLog,
    new EnvironmentPrismaDailyConnectorQuota(prisma),
    now,
  )
  const artifacts = voiceArtifactStore ?? new PrismaVoiceArtifactStore(prisma)
  ...
  app.post(`${gclBase}/connectors/:connectorId/runs`, gclOwnerAuth(gclOwnerToken), asyncRoute(async (request, response) => {
    ...
    const artifactAudit = await auditLog.append({
      type: 'voice.artifact.created', connectorId, product: scope.product, workspaceId: scope.workspaceId, actor,
  ...
  const artifactBase = `${gclBase}/voice-artifacts`
  app.get(`${artifactBase}/:recordId`, gclOwnerAuth(gclOwnerToken), asyncRoute(async (request, response) => {
    ...
    if (!artifact) return response.status(404).json({ error: 'VOICE_ARTIFACT_NOT_FOUND', code: 'voice_artifact_not_found' })
  ...
  app.post(`${artifactBase}/:recordId/approval`, gclOwnerAuth(gclOwnerToken), asyncRoute(async (request, response) => {
    const scope = workspaceScopeFrom(request)
    const actor = ownerActorFrom(request)
    const decision = voiceArtifactApprovalFrom(request.body)
    const artifact = await artifacts.decide({ ...scope, id: recordIdFrom(request), actor, decision: decision.decision, now: now() })
    if (!artifact) return response.status(404).json({ error: 'VOICE_ARTIFACT_NOT_FOUND', code: 'voice_artifact_not_found' })
    const artifactAudit = await auditLog.append({
      type: decision.decision === 'approved' ? 'voice.artifact.approved' : 'voice.artifact.rejected', connectorId: artifact.connectorId,
      product: scope.product, workspaceId: scope.workspaceId, actor, scopes: ['voice:artifact:approve'], costCapCents: 0, requestedItems: 0, occurredAt: now().toISOString(),
      detail: { artifactId: artifact.id, kind: artifact.kind, contentHash: artifact.contentHash, approvalState: artifact.approvalState, runAuditHash: artifact.runAuditHash, autoPublish: false },
    })
