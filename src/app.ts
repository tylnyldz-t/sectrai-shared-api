...
import { EnvironmentPrismaConnectorQuota, GAME_ENGINE_DAILY_QUOTA_ENV, GCL_USAGE_MODULE_ID, THREE_D_DAILY_QUOTA_ENV } from './gcl/quota.js'
...
export function createApp({ prisma = new PrismaClient(), now = () => new Date(), gclRunner, environment = process.env, gclOwnerToken = environment.GCL_OWNER_TOKEN }: AppOptions = {}) {
  ...
  const governedRunner = gclRunner ?? new GovernedConnectorRunner(
    new ConnectorRegistry([...syntheticThreeDConnectorsFromEnvironment(environment), gameEngineConnectorFromEnvironment(environment)]),
    new PrismaHashChainAuditLog(prisma),
    new EnvironmentPrismaConnectorQuota(prisma, environment, {
      'text-to-3d': THREE_D_DAILY_QUOTA_ENV,
      'image-text-to-3d': THREE_D_DAILY_QUOTA_ENV,
      'game-engine': GAME_ENGINE_DAILY_QUOTA_ENV,
    }),
    now,
  )
