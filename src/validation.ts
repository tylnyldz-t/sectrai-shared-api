...
export type VoiceArtifactApprovalMutation = { decision: 'approved' | 'rejected' }
...
export function voiceArtifactApprovalFrom(body: unknown): VoiceArtifactApprovalMutation {
  const input = exactObject(body, ['decision'], 'INVALID_VOICE_ARTIFACT_APPROVAL')
  if (input.decision !== 'approved' && input.decision !== 'rejected') throw Object.assign(new Error('INVALID_VOICE_ARTIFACT_DECISION'), { status: 422 })
