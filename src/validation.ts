...
function opaqueCorrelationId(value: unknown): string {
  if (typeof value !== 'string' || !/^[a-zA-Z0-9:_-]{1,120}$/.test(value)) throw Object.assign(new Error('INVALID_CONNECTOR_CORRELATION_ID'), { status: 422 })
  return value
}
...
export function connectorRunFrom(body: unknown): ConnectorRunMutation {
  ...
  return {
    ...
    correlationId: opaqueCorrelationId(input.correlationId),
  }
}
