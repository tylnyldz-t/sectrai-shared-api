...
counted after a failed run. Audit append receipts are re-hashed before the run
can advance. `text-to-3d` and `image-text-to-3d` share the single workspace-level
3D daily allowance configured by `GCL_3D_DAILY_*`.
