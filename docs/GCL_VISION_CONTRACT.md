...
The validator rejects missing, extra, inherited, hidden, accessor-backed, or altered maker–checker bindings before the review audit append; v1 through v7 packets are deliberately non-reviewable. The comparison is deliberately conservative: where two accepted ASCII IDs differ only by case, the module denies self-review rather than treating them as independent. D8 adds neither a credential nor authentication claim, durable replay state, send/apply authority, provider integration, or a live mode. D9 through D13 supersede v8 for newly issued packets.
...
D10 does not normalize, reinterpret, repair, or retain raw document text. It adds no credential, OCR/provider client, signature, persistence, replay control, approval authority, apply/send path, or live mode; it only denies an encoding-ambiguous synthetic value before it can enter the in-process integrity material. D11 through D13 supersede v10 for newly issued packets.
...
Review revalidation requires the exact, integrity-bound time-binding fields. Missing, extra, inherited, hidden, accessor-backed, or altered bindings are rejected before audit append; v1 through v10 packets are deliberately non-reviewable. D11 is not clock attestation, a signature, credential, replay mechanism, durable review state, apply/send authorization, provider integration, or live mode. It only denies an object-injected or unstable in-process clock before it can affect synthetic timing. D12 and D13 supersede v11 for newly issued packets.
...
New proposals use `synthetic-document-review-packet-v13`. Its integrity material adds a fixed `proxyBoundaryBinding`: `proxyDetection: "node-util-types-isProxy"`, `proxyObjectsAccepted: false`, and `proxyArraysAccepted: false`.
...
D13 is a local Node runtime boundary, not a cross-runtime proof, a signature, credential, provider capability, replay control, durable state change, apply/send authorization, or OCR path. It introduces no network client or raw document handling.
