...
## D23 — review-audit handoff snapshot boundary

New proposals use `synthetic-document-review-packet-v23`. Its integrity material adds a fixed `auditEventBoundaryBinding`: `auditEvent: "adapter-created-frozen-own-data-only"`, `auditEventScope: "single-read-validated-review-context-only"`, and `auditEventMutationAccepted: false`.

Before proposal revalidation, the independent-review entry point rejects a Proxy review context and reads `product` and `workspaceId` exactly once into a validated scope snapshot. It never reads those context properties again when it creates the audit event. The event is then assembled only from normalized strings, fixed connector metadata, the already-revalidated proposal, and a fixed no-send handoff marker. Its top-level record, `scopes` array, and `detail` record are frozen with the module-captured `Object.freeze` before the descriptor-validated audit method receives them. An audit implementation therefore cannot mutate the supplied reference to switch product/workspace, add a scope, or turn `rawContentIncluded` on after validation.

D23 is a local handoff-integrity boundary, not an assertion that an audit implementation persists the same event, a clean-realm guarantee, signature, credential, provider capability, replay control, durability proof, persistence action, apply/send authorization, or live OCR path. It accepts no raw document content, makes no network request, and adds no migration. Missing, extra, inherited, hidden, accessor-backed, altered, or v1–v22 packets are denied before a review audit append. D24 supersedes it for newly issued packets.

## D24 — descriptor-bound review-context boundary

New proposals use `synthetic-document-review-packet-v24`. Its integrity material adds a fixed `reviewContextBoundaryBinding`: `contextMembers: "own-enumerable-data-properties-read-once"`, `inheritedOrAccessorMembersAccepted: false`, and `proxyClockFunctionAccepted: false`.

Before an independent review or standalone packet validation reads a scope or invokes its clock, the adapter requires a non-Proxy plain record and obtains `product`, `workspaceId`, and `now` only from their own enumerable data descriptors. An inherited member, non-enumerable member, getter/setter, missing member, or Proxy clock function is rejected before that member is read or invoked. The three values are captured once into a local snapshot; the clock is then called without the mutable context as its receiver and must still satisfy D11's exact built-in-`Date` boundary. The same descriptor-read clock boundary is used while a synthetic proposal is preflighted and issued.

D24 does not claim that a direct clock function is trustworthy, that the runtime realm was clean before module load, or that an audit sink is durable. It is not a signature, credential, provider capability, replay control, persistence action, apply/send authorization, or live OCR path. It accepts no raw document content, makes no network request, and adds no migration. Missing, extra, inherited, hidden, accessor-backed, altered, or v1–v23 packets are denied before a review audit append. D25 supersedes it for newly issued packets.

## D25 — literal owner-approval boundary

New proposals use `synthetic-document-review-packet-v25`. Its integrity material adds a fixed `ownerApprovalBoundaryBinding`: `approvalValue: "literal-boolean-true-only"`, `truthyValuesAccepted: false`, and `approvalCheckedBeforeReview: true`.

Independent review accepts the owner gate only when the supplied value is the primitive Boolean `true`; it rejects values such as `1`, `"true"`, `{}`, and `new Boolean(true)` before it validates the packet, reads a reviewer, invokes a clock, or appends audit. The common governed-runner owner gate applies the same literal-`true` requirement before preflight, audit reservation, or quota reservation. This prevents an untyped caller from widening an owner-only route through JavaScript truthiness. The fixed review-packet binding is descriptor-validated and integrity-bound; missing, extra, inherited, hidden, accessor-backed, altered, or v1–v24 packets are denied before review audit append.

D25 is not authentication, a credential, a signature, durable approval state, replay protection, provider capability, persistence action, apply/send authorization, or a live OCR path. It accepts no raw document content, makes no network request, and adds no migration. It only preserves the existing outer-owner gate as an explicit, fail-closed Boolean boundary. D26 supersedes it for newly issued packets.

## D26 — closed-set review-decision boundary

New proposals use `synthetic-document-review-packet-v26`. Its integrity material adds a fixed `reviewDecisionBoundaryBinding`: `decisionValue: "exact-approved-or-rejected-string-only"`, `coercionAccepted: false`, and `decisionCheckedBeforeReviewerOrClock: true`.

After the literal-`true` owner gate, independent review accepts only the primitive strings `"approved"` and `"rejected"`. Case variants, whitespace variants, empty strings, numbers, Booleans, boxed strings, records, symbols, and every other value are rejected before reviewer validation, context/clock reads, packet validation, or audit append. The fixed binding is descriptor-validated and integrity-bound; missing, extra, inherited, hidden, accessor-backed, altered, or v1–v25 packets are denied before a review audit append.

D26 is not authentication, a credential, a signature, durable approval state, replay protection, provider capability, persistence action, apply/send authorization, or a live OCR path. It accepts no raw document content, makes no network request, and adds no migration. It only closes the runtime decision enum at the synthetic review boundary.
