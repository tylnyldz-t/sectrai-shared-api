...
The checks run before a review audit append, including for accidentally altered in-process packets whose integrity digest no longer matches. D5 intentionally does not turn the unkeyed digest into a signature or durable replay control; D6 and D7 supersede it for newly issued packets and a future scoped host must still resolve state, revocation, and one-time decisions before any separate action.
