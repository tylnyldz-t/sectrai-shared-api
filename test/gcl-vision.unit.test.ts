...
test('GM2 run requires owner gate, scope, cost cap, quota, consent, and creates only a masked pending proposal', async () => {
  ...
  assert.equal(proposal.reviewPacket.version, 'synthetic-document-review-packet-v26')
  ...
  assert.deepEqual(proposal.reviewPacket.reviewContextBoundaryBinding, { contextMembers: 'own-enumerable-data-properties-read-once', inheritedOrAccessorMembersAccepted: false, proxyClockFunctionAccepted: false })
  assert.deepEqual(proposal.reviewPacket.ownerApprovalBoundaryBinding, { approvalValue: 'literal-boolean-true-only', truthyValuesAccepted: false, approvalCheckedBeforeReview: true })
  assert.deepEqual(proposal.reviewPacket.reviewDecisionBoundaryBinding, { decisionValue: 'exact-approved-or-rejected-string-only', coercionAccepted: false, decisionCheckedBeforeReviewerOrClock: true })
...
test('D5 requires a causally coherent review timeline and refuses review deadlines beyond consent before audit append', async () => {
  ...
  assert.equal(result.data.proposal.reviewPacket.version, 'synthetic-document-review-packet-v26')
...
test('D8 canonical maker-checker identity remains enforced under D26 packets', async () => {
  const audit = new InMemoryHashChainAuditLog()
  const quota = new TestQuota()
  const runner = new GovernedConnectorRunner(new ConnectorRegistry([configuredConnector(60, 300)]), audit, quota, now)
  const result = await runner.run({ connectorId: VISION_DOCUMENT_FIELD_EXTRACTION_CONNECTOR_ID, input, ...context }) as ConnectorResult<DocumentFieldExtractionData>
...
test('D19 binds the module-captured Proxy inspector and ignores a late node:util inspector replacement under D26 packets', async () => {
  const audit = new InMemoryHashChainAuditLog()
...
test('D23 snapshots the review scope once and freezes the audit handoff against downstream mutation', async () => {
  ...
  const stableContext = { product: context.product, workspaceId: context.workspaceId, now: context.now }
  ...
  const reviewed = await independentlyReviewSyntheticDocumentProposal(clone(), 'approved', true, 'checker@example.test', mutationAudit, stableContext)
...
test('D24 descriptor-binds review scope and clock members before they can alter or invoke a review', async () => {
  const proposal = (await configuredConnector(60, 300).run(input, context)).data.proposal
  const clone = () => JSON.parse(JSON.stringify(proposal)) as typeof proposal
  assert.equal(proposal.reviewPacket.version, 'synthetic-document-review-packet-v26')
  assert.deepEqual(proposal.reviewPacket.reviewContextBoundaryBinding, {
  ...
  let appendCalls = 0
  const audit = { async append(): Promise<{ hash: string }> { appendCalls += 1; return { hash: '2'.repeat(64) } } }
  const missingBinding = clone()
  delete (missingBinding.reviewPacket as { reviewContextBoundaryBinding?: unknown }).reviewContextBoundaryBinding
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(missingBinding, 'approved', true, 'checker@example.test', audit, context), (error: unknown) => error instanceof ConnectorInputError && error.message === 'INVALID_DOCUMENT_REVIEW_CONTEXT_BOUNDARY_BINDING')

  const extraBindingField = clone()
  ;(extraBindingField.reviewPacket.reviewContextBoundaryBinding as { extension?: unknown }).extension = 'forbidden'
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(extraBindingField, 'approved', true, 'checker@example.test', audit, context), (error: unknown) => error instanceof ConnectorInputError && error.message === 'UNEXPECTED_DOCUMENT_REVIEW_CONTEXT_BOUNDARY_BINDING_FIELD')

  const alteredBinding = clone()
  alteredBinding.reviewPacket.reviewContextBoundaryBinding.inheritedOrAccessorMembersAccepted = true as never
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(alteredBinding, 'approved', true, 'checker@example.test', audit, context), (error: unknown) => error instanceof ConnectorInputError && error.message === 'INVALID_DOCUMENT_REVIEW_CONTEXT_BOUNDARY_BINDING')

  const legacyPacket = clone()
  legacyPacket.reviewPacket.version = 'synthetic-document-review-packet-v23' as never
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(legacyPacket, 'approved', true, 'checker@example.test', audit, context), (error: unknown) => error instanceof ConnectorInputError && error.message === 'DOCUMENT_REVIEW_PACKET_VERSION_UNSUPPORTED')
  assert.equal(appendCalls, 0)

  let getterReads = 0
  const accessorScope = {
    get product(): string { getterReads += 1; throw new Error('REVIEW_CONTEXT_GETTER_MUST_NOT_RUN') },
    workspaceId: context.workspaceId,
    now: context.now,
  }
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(clone(), 'approved', true, 'checker@example.test', audit, accessorScope), (error: unknown) => error instanceof ConnectorInputError && error.message === 'INVALID_DOCUMENT_REVIEW_CONTEXT')
  assert.equal(getterReads, 0)

  const inheritedScope = Object.create({ product: context.product }) as { workspaceId: string; now: () => Date }
  inheritedScope.workspaceId = context.workspaceId
  inheritedScope.now = context.now
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(clone(), 'approved', true, 'checker@example.test', audit, inheritedScope as unknown as Pick<ConnectorRunContext, 'product' | 'workspaceId' | 'now'>), (error: unknown) => error instanceof ConnectorInputError && error.message === 'INVALID_DOCUMENT_REVIEW_CONTEXT')

  const hiddenScope = { product: context.product, now: context.now }
  Object.defineProperty(hiddenScope, 'workspaceId', { value: context.workspaceId, enumerable: false })
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(clone(), 'approved', true, 'checker@example.test', audit, hiddenScope as unknown as Pick<ConnectorRunContext, 'product' | 'workspaceId' | 'now'>), (error: unknown) => error instanceof ConnectorInputError && error.message === 'INVALID_DOCUMENT_REVIEW_CONTEXT')

  let proxyClockCalls = 0
  const proxyClock = new Proxy(() => { proxyClockCalls += 1; return now() }, {
    apply() { proxyClockCalls += 1; return now() },
  })
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(clone(), 'approved', true, 'checker@example.test', audit, { product: context.product, workspaceId: context.workspaceId, now: proxyClock }), (error: unknown) => error instanceof ConnectorInputError && error.message === 'INVALID_DOCUMENT_REVIEW_TIME')
  assert.equal(proxyClockCalls, 0)
  assert.equal(appendCalls, 0)

  const reviewed = await independentlyReviewSyntheticDocumentProposal(clone(), 'approved', true, 'checker@example.test', audit, { product: context.product, workspaceId: context.workspaceId, now: context.now })
  assert.equal(reviewed.auditHash, '2'.repeat(64))
  assert.equal(appendCalls, 1)
  assert.equal(reviewed.mesaEvidenceHandoff.sent, false)
})

test('D25 accepts only literal true for owner approval and binds that no-coercion boundary before audit', async () => {
  const proposal = (await configuredConnector(60, 300).run(input, context)).data.proposal
  const clone = () => JSON.parse(JSON.stringify(proposal)) as typeof proposal
  assert.equal(proposal.reviewPacket.version, 'synthetic-document-review-packet-v26')
  assert.deepEqual(proposal.reviewPacket.ownerApprovalBoundaryBinding, {
  ...
  let appendCalls = 0
  const audit = { async append(): Promise<{ hash: string }> { appendCalls += 1; return { hash: '3'.repeat(64) } } }
  const missingBinding = clone()
  delete (missingBinding.reviewPacket as { ownerApprovalBoundaryBinding?: unknown }).ownerApprovalBoundaryBinding
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(missingBinding, 'approved', true, 'checker@example.test', audit, context), (error: unknown) => error instanceof ConnectorInputError && error.message === 'INVALID_DOCUMENT_REVIEW_OWNER_APPROVAL_BOUNDARY_BINDING')

  const extraBindingField = clone()
  ;(extraBindingField.reviewPacket.ownerApprovalBoundaryBinding as { extension?: unknown }).extension = 'forbidden'
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(extraBindingField, 'approved', true, 'checker@example.test', audit, context), (error: unknown) => error instanceof ConnectorInputError && error.message === 'UNEXPECTED_DOCUMENT_REVIEW_OWNER_APPROVAL_BOUNDARY_BINDING_FIELD')

  const alteredBinding = clone()
  alteredBinding.reviewPacket.ownerApprovalBoundaryBinding.truthyValuesAccepted = true as never
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(alteredBinding, 'approved', true, 'checker@example.test', audit, context), (error: unknown) => error instanceof ConnectorInputError && error.message === 'INVALID_DOCUMENT_REVIEW_OWNER_APPROVAL_BOUNDARY_BINDING')

  const legacyPacket = clone()
  legacyPacket.reviewPacket.version = 'synthetic-document-review-packet-v24' as never
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(legacyPacket, 'approved', true, 'checker@example.test', audit, context), (error: unknown) => error instanceof ConnectorInputError && error.message === 'DOCUMENT_REVIEW_PACKET_VERSION_UNSUPPORTED')

  for (const truthyApproval of [1, 'true', {}, new Boolean(true)] as const) {
    await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(clone(), 'approved', truthyApproval as never, 'checker@example.test', audit, context), OwnerGateError)
  }
  assert.equal(appendCalls, 0)

  const quota = new TestQuota()
  const runner = new GovernedConnectorRunner(new ConnectorRegistry([configuredConnector(60, 300)]), audit, quota, now)
  await assert.rejects(() => runner.run({ connectorId: VISION_DOCUMENT_FIELD_EXTRACTION_CONNECTOR_ID, input, ...context, ownerApproved: 1 as never }), OwnerGateError)
  assert.equal(appendCalls, 0)
  assert.equal(quota.requests.length, 0)

  const reviewed = await independentlyReviewSyntheticDocumentProposal(clone(), 'approved', true, 'checker@example.test', audit, context)
  assert.equal(reviewed.auditHash, '3'.repeat(64))
  assert.equal(reviewed.mesaEvidenceHandoff.sent, false)
  assert.equal(appendCalls, 1)
})

test('D26 accepts only exact review decisions, validates its packet binding, and stops before reviewer or clock work', async () => {
  const proposal = (await configuredConnector(60, 300).run(input, context)).data.proposal
  const clone = () => JSON.parse(JSON.stringify(proposal)) as typeof proposal
  assert.equal(proposal.reviewPacket.version, 'synthetic-document-review-packet-v26')
  assert.deepEqual(proposal.reviewPacket.reviewDecisionBoundaryBinding, {
    decisionValue: 'exact-approved-or-rejected-string-only',
    coercionAccepted: false,
    decisionCheckedBeforeReviewerOrClock: true,
  })

  let appendCalls = 0
  let clockCalls = 0
  const audit = { async append(): Promise<{ hash: string }> { appendCalls += 1; return { hash: '4'.repeat(64) } } }
  const clockContext = { product: context.product, workspaceId: context.workspaceId, now: () => { clockCalls += 1; return now() } }

  const missingBinding = clone()
  delete (missingBinding.reviewPacket as { reviewDecisionBoundaryBinding?: unknown }).reviewDecisionBoundaryBinding
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(missingBinding, 'approved', true, 'checker@example.test', audit, clockContext), (error: unknown) => error instanceof ConnectorInputError && error.message === 'INVALID_DOCUMENT_REVIEW_DECISION_BOUNDARY_BINDING')

  const extraBindingField = clone()
  ;(extraBindingField.reviewPacket.reviewDecisionBoundaryBinding as { extension?: unknown }).extension = 'forbidden'
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(extraBindingField, 'approved', true, 'checker@example.test', audit, clockContext), (error: unknown) => error instanceof ConnectorInputError && error.message === 'UNEXPECTED_DOCUMENT_REVIEW_DECISION_BOUNDARY_BINDING_FIELD')

  const alteredBinding = clone()
  alteredBinding.reviewPacket.reviewDecisionBoundaryBinding.coercionAccepted = true as never
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(alteredBinding, 'approved', true, 'checker@example.test', audit, clockContext), (error: unknown) => error instanceof ConnectorInputError && error.message === 'INVALID_DOCUMENT_REVIEW_DECISION_BOUNDARY_BINDING')

  const legacyPacket = clone()
  legacyPacket.reviewPacket.version = 'synthetic-document-review-packet-v25' as never
  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(legacyPacket, 'approved', true, 'checker@example.test', audit, clockContext), (error: unknown) => error instanceof ConnectorInputError && error.message === 'DOCUMENT_REVIEW_PACKET_VERSION_UNSUPPORTED')

  for (const invalidDecision of ['APPROVED', 'approved ', '', 1, true, new String('approved'), { value: 'approved' }, Symbol('approved')] as const) {
    await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(clone(), invalidDecision, true, 'checker@example.test', audit, clockContext), (error: unknown) => error instanceof ConnectorInputError && error.message === 'INVALID_DOCUMENT_REVIEW_DECISION')
  }
  assert.equal(clockCalls, 4)
  assert.equal(appendCalls, 0)

  await assert.rejects(() => independentlyReviewSyntheticDocumentProposal(clone(), 'approved', false, 'checker@example.test', audit, clockContext), OwnerGateError)
  assert.equal(clockCalls, 4)
  assert.equal(appendCalls, 0)

  const reviewed = await independentlyReviewSyntheticDocumentProposal(clone(), 'rejected', true, 'checker@example.test', audit, clockContext)
  assert.equal(reviewed.decision, 'rejected')
  assert.equal(reviewed.ownerReview.status, 'rejected')
  assert.equal(reviewed.auditHash, '4'.repeat(64))
